...
void fillwithreversediamond()
{
    ...
    set_coordinates();
    for(int i=0;i<11;i++){
        set_static_ball(255,0,0,i,i+2);
        set_static_ball(255,0,0,i,-i+2+20);
    }
    for(int i=0;i<10;i++){
        set_static_ball(0,255,0,i,i+3);
        set_static_ball(0,255,0,i,-i+3+18);
    }
    for(int i=0;i<9;i++){
        set_static_ball(0,0,255,i,i+4);
        set_static_ball(0,0,255,i,-i+4+16);
    }
    for(int i=0;i<8;i++){
        set_static_ball(0,255,255,i,i+5);
        set_static_ball(0,255,255,i,-i+5+14);
    }
    for(int i=0;i<7;i++){
        set_static_ball(0,0,255,i,i+6);
        set_static_ball(0,0,255,i,-i+6+12);
    }
    for(int i=0;i<6;i++){
        set_static_ball(0,255,0,i,i+7);
        set_static_ball(0,255,0,i,-i+7+10);
    }
    for(int i=0;i<5;i++){
        set_static_ball(255,0,255,i,i+8);
        set_static_ball(255,0,255,i,-i+8+8);
    }
    for(int i=0;i<4;i++){
        set_static_ball(255,255,0,i,i+9);
        set_static_ball(255,255,0,i,-i+9+6);
    }
    for(int i=0;i<3;i++){
        set_static_ball(0,255,255,i,i+10);
        set_static_ball(0,255,255,i,-i+10+4);
    }
    for(int i=0;i<2;i++){
        set_static_ball(255,0,0,i,i+11);
        set_static_ball(255,0,0,i,-i+11+2);
    }
    set_static_ball(0,255,0,0,12);
...
void updateMusic()
{
    ...
    iSetVolume(desiredHandle, currentVolume);
}
...
void iMouse(int button, int state, int mx, int my)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        else if(screenCount==2)
        {
            // Code for high-score screen
            if(mx >= 120 && mx <= 380 && my >= 400 && my <= 450) {
                screenCount = 0; // Back to main menu
            }
            else if(mx >= 120 && mx <= 380 && my >= 320 && my <= 370) {
                screenCount = 0; // Back to main menu
            }
